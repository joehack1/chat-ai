...
class ChatAI {
                    ...
                    <div class="wrapper">
                        ...
                        <div class="details">
                            <div class="date" title="${
                              message.date
                            }">${this.formatElapsedTime(message.date)}</div>
                            <div class="text">
                                ${message.content
                                  .replace(/(?:\r\n|\r|\n)/g, "<br>")
                                  .replace(
  ...
  createNewConversation(title = null) {
    ...
    this.container.querySelector(".content .messages").innerHTML =
      '<div class="conversation-title"><h2><span class="text">' +
      title +
      '</span><i class="fa-solid fa-pencil edit"></i><i class="fa-solid fa-trash delete"></i></h2></div>';
    this._conversationTitleClickHandler();
    this.container.querySelector(".message-form input").focus();
    this.updateTitle();
    return index - 1;
                    ...
                    <div class="content">
                        ...
                        <div class="body">
                            <form class="settings-form" action="">
                                ...
                                <input type="number" name="max_tokens" id="max_tokens" value="${
                                  self.maxTokens
                                }">
                                <div class="msg"></div>
                            </form>
  ...
  _welcomePageTemplate() {
    return `
            <div class="welcome">
                <h1>Joel Chat AI <span class="ver">${this.options.version}</span></h1>                    
                <p>Made with love by <a href="" target="_blank">Joel Loter</a> &lt;3</p>
  ...
  _conversationTitleClickHandler() {
    this.container.querySelector(".conversation-title i.edit").onclick = () => {
      ...
      this.container.querySelector(".conversation-title .text").onblur = () =>
        update();
      this.container.querySelector(".conversation-title .text").onkeydown = (
        event
      ) => {
        if (event.keyCode == 13) {
          event.preventDefault();
    ...
    this.container.querySelector(".conversation-title i.delete").onclick =
      () => {
        if (confirm("Are you sure you want to delete this conversation?")) {
          ...
          this.container
            .querySelector(".conversations .list .conversation.selected")
            .remove();
          this.updateTitle();
          if (!this.container.querySelector(".content .welcome")) {
            this.container
              .querySelector(".content")
              .insertAdjacentHTML("afterbegin", this._welcomePageTemplate());
          }
          ...
          if (json !== undefined) {
            ...
            this.conversations.forEach((conversation, index) => {
              this.container
                ...
                .insertAdjacentHTML(
                  "beforeend",
                  `<a class="conversation" href="#" data-id="${index}" title="${conversation.name}"><i class="fa-regular fa-message"></i>${conversation.name}</a>`
                );
            });
            this._conversationClickHandlers();
            this._openDatabaseEventHandlers();
  ...
  _eventHandlers() {
    ...
    this.container.querySelector(".conversations .new-conversation").onclick = (
      event
    ) => {
      event.preventDefault();
      this.selectedConversationIndex = this.createNewConversation();
    ...
    setInterval(() => {
      this.container.querySelectorAll("[data-date]").forEach((element) => {
        element.innerHTML = this.formatElapsedTime(
...
new ChatAI({
  container: ".chat-ai",
  api_key: "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",
  model: "gpt-3.5-turbo",
});
